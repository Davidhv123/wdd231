...
function displayItems(projects) {
  projects.forEach(x => {
    ...
    thedate.classList = "date-xp"
    thedate.innerText = x.monthYear
    thecard.appendChild(thedate)

    //build the tool
    const thetool = document.createElement("p")
    thetool.classList = "tool-xp"
    thetool.innerText = x.herramienta
    thecard.appendChild(thetool)
