...
// FUNCION PARA TRAER EL JSON AQUI CON FECTH Y ASYN Y AWAIT.
async function loadmembers() {
  try {
    ...
    document.getElementById("companies-container").innerHTML = `<p>Error al cargar datos</p>`;
  }
}
...
const displayEntities = (entityData) => {
  ...
  entityData.forEach((company) => {
    ...
    pAddress.innerHTML = `<strong>📍 Address: </strong> <br>${company.address}`;
    pPhone.innerHTML = `<strong>📞 Phone: </strong> <br>${company.phone}`;
    pInfo.innerHTML = `<strong>💡 Info: </strong> <br>${company.additional_info}`;
